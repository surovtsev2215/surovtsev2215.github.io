...
echo Шаг 2/4: Проверка сборки
echo Шаг 3/4: Запуск локального сайта
echo Шаг 4/4: Автооткрытие сайта в браузере
echo ================================================
echo.
...
echo [3/5] Проверка Firebase конфигурации
if not exist ".env.local" goto :fail_env
findstr /R /I "^VITE_FIREBASE_API_KEY=." ".env.local" >nul || goto :fail_env
findstr /R /I "^VITE_FIREBASE_AUTH_DOMAIN=." ".env.local" >nul || goto :fail_env
findstr /R /I "^VITE_FIREBASE_PROJECT_ID=." ".env.local" >nul || goto :fail_env
findstr /R /I "^VITE_FIREBASE_STORAGE_BUCKET=." ".env.local" >nul || goto :fail_env
findstr /R /I "^VITE_FIREBASE_MESSAGING_SENDER_ID=." ".env.local" >nul || goto :fail_env
findstr /R /I "^VITE_FIREBASE_APP_ID=." ".env.local" >nul || goto :fail_env

echo [4/5] Проверка production-сборки
...
:: #endregion
echo.
echo ERROR: Firebase настройки не заполнены в web\frontend\.env.local
echo Для online режима добавьте все VITE_FIREBASE_* переменные.
echo Подробности: "%LOG_FILE%"
echo Нажмите любую клавишу, чтобы закрыть окно...
pause >nul
call :finalize_log
...
findstr /R /I "^VITE_FIREBASE_API_KEY=." "%ROOT_DIR%\web\frontend\.env.local" >nul || exit /b 1
findstr /R /I "^VITE_FIREBASE_AUTH_DOMAIN=." "%ROOT_DIR%\web\frontend\.env.local" >nul || exit /b 1
findstr /R /I "^VITE_FIREBASE_PROJECT_ID=." "%ROOT_DIR%\web\frontend\.env.local" >nul || exit /b 1
findstr /R /I "^VITE_FIREBASE_STORAGE_BUCKET=." "%ROOT_DIR%\web\frontend\.env.local" >nul || exit /b 1
findstr /R /I "^VITE_FIREBASE_MESSAGING_SENDER_ID=." "%ROOT_DIR%\web\frontend\.env.local" >nul || exit /b 1
findstr /R /I "^VITE_FIREBASE_APP_ID=." "%ROOT_DIR%\web\frontend\.env.local" >nul || exit /b 1
